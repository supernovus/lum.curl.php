 ...
 *
 * For simple things this works alright, but it's pretty limited.
 * Right now it only has GET and POST support. Like I said, limited.
 *
 * For something more complete, you could try Guzzle, 'guzzlehttp/guzzle'.
 * Or for web services, try Lum\Webservice, 'lum/lum-webservice'.
 */
class Curl
{
  ...
  protected function get_map ($name)
  {
    ...
    {
      throw new CurlException('Unknown option.');
    }
  }
  ...
  public function get ($url)
  {
    ...
    if ($output === False || $output == '')
    {
      ...
      if ($this->strict)
      {
        throw new CurlException('cURL request failed: '.json_encode($output));
      }
    }
    $this->end();
    return $output;
